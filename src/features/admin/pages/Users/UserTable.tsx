import { MoreHorizontal, ShieldCheck, ShieldOff, Trash2, Users, ChevronUp, ChevronDown } from "lucide-react";
import { Badge } from "../../../../app/components/ui/Badge";
import { Pagination } from "../../../../app/components/ui/Pagination";
import { ScrollTable } from "../../../../app/components/ui/ScrollTable";
...
function TH({ label, sortKey, active, dir, onToggle }: { label: string; sortKey?: string; active?: boolean; dir?: 'asc' | 'desc'; onToggle?: () => void }) {
  return (
...
export function UserTable({ paginated, filtered, sortKey, dir, onToggle, activeMenu, onMenuToggle, onSuspend, onActivate, onDelete, page, totalPages, onPage, search }: UserTableProps) {
  return (
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-muted border-b border-border">
              <TH label="User" sortKey="name" active={sortKey === 'name'} dir={dir} onToggle={() => onToggle('name')} />
              <TH label="Role" sortKey="role" active={sortKey === 'role'} dir={dir} onToggle={() => onToggle('role')} />
              <TH label="Department" sortKey="department" active={sortKey === 'department'} dir={dir} onToggle={() => onToggle('department')} />
              <TH label="Joined" sortKey="joined" active={sortKey === 'joined'} dir={dir} onToggle={() => onToggle('joined')} />
              <TH label="Status" sortKey="status" active={sortKey === 'status'} dir={dir} onToggle={() => onToggle('status')} />
              <TH label="" />
            </tr>
          </thead>
              ...
              <tr key={u.id} className="bg-card hover:bg-muted transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="relative" onClick={e => e.stopPropagation()}>
                    <button onClick={() => onMenuToggle(u.id)}
                    ...
                    {activeMenu === u.id && (
                      <div className="absolute right-0 top-full mt-1 min-w-[190px] rounded-xl shadow-xl z-30 overflow-hidden bg-card border border-border">
                        {u.status === 'Active' ? (
                          ...
                          <button onClick={() => onActivate(u.id)}

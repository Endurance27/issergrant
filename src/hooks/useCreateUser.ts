...
import type { CreateUserInput } from '../gql/graphql';
import { CREATE_USER_MUTATION } from '@/apollo/mutations';
import { useMutation } from '@apollo/client/react';
import { Mutation, MutationCreateUserArgs } from '@/gql/schema-types';
...
export function useCreateUser() {
  ...
  const createUser = async (input: CreateUserInput) => {
    ...
    try {
      const mappedInput = {
        ...input,
        role: (roleToEnum[input.role] ??
          input.role.toLowerCase()) as CreateUserInput['role'],
      };

...
export function CreateGuest() {
  ...
  const formik = useFormik<CreateGuestFormValues>({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      const payload = await createGuest({
        ...values,
        assignedResearcherId: currentUser?.UserId ?? "",
      });
      if (!payload) {
        setApiError("Failed to create guest. Please try again.");
        return;
      }
      toast(`${payload.user.name} added as guest`, "success");
      resetForm();
      setTempResult({
  ...
  return (
    <div>
      <PageHeader
        title="Add Guest Collaborator"
        subtitle="Invite an external collaborator to view assigned funding calls and proposals"
      />
        ...
        <div className="rounded-2xl bg-card border border-border p-6">
          <form onSubmit={formik.handleSubmit} noValidate className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Kofi Agyeman"
                  className={inputCls}
                  disabled={loading}
                />
                {formik.touched.name && formik.errors.name && (
                  <p className={errCls}>{formik.errors.name}</p>
                )}
              </div>
              <div>
                <label className={labelCls}>
                ...
                <input
                  ...
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  placeholder="guest@institution.edu"
                  className={inputCls}
                  disabled={loading}
                />
                {formik.touched.email && formik.errors.email && (
                  <p className={errCls}>{formik.errors.email}</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className={labelCls}>
                  Staff / Guest ID <span className="text-red-500">*</span>
                </label>
                <input
                  name="staffId"
                  ...
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  placeholder="e.g. GUE-001"
                  className={inputCls}
                  disabled={loading}
                />
                {formik.touched.staffId && formik.errors.staffId && (
                  <p className={errCls}>{formik.errors.staffId}</p>
                )}
              </div>
              <div>
                <label className={labelCls}>
                ...
                {formik.touched.phoneContact && formik.errors.phoneContact && (
                  <p className={errCls}>{formik.errors.phoneContact}</p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
                placeholder="e.g. Lead statistician for the household survey component..."
                className={`${inputCls} resize-none`}
                disabled={loading}
              />
              {formik.touched.notes && formik.errors.notes && (
                <p className={errCls}>{formik.errors.notes}</p>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-muted border border-border text-[12px] text-muted-foreground">
              <span className="w-5 h-5 rounded-full bg-primary/10 text-primary flex items-center justify-center flex-shrink-0 text-[10px] font-bold">
                i
              </span>
              This guest will be assigned to you automatically. Role is set to
              "Guest".
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 p-3 rounded-xl bg-muted border border-border">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border border-border bg-card hover:bg-muted text-foreground transition-colors flex-shrink-0"
                >
                  {copied ?
                    <>
            ...
            <div className="p-3 rounded-xl bg-secondary border border-primary/20">
              <p className="text-xs text-primary">
                Share this temporary password with the guest. They will be

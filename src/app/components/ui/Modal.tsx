...
export function Modal({ open, onClose, title, children, width = 560 }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-[rgba(15,23,42,0.5)] backdrop-blur-[2px]" onClick={onClose} />
      <div className="relative rounded-xl shadow-2xl w-full overflow-hidden bg-card border border-border" style={{ maxWidth: width }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          ...
          <button
            onClick={onClose}
            className="flex items-center justify-center w-7 h-7 rounded-lg transition-colors hover:bg-muted text-muted-foreground"
          >
            <X size={16} />
          </button>
        </div>

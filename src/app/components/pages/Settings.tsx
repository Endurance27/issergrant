          ...
          <div className="space-y-1">
            {tabs.map(tab => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-left text-[13px] ${activeTab === tab.id ? 'bg-secondary text-primary font-bold' : 'text-muted-foreground font-medium hover:bg-muted'}`}
              >
                {tab.icon} {tab.label}
              </button>
            ))}
            ...
            <div className="rounded-2xl p-6 bg-card border border-border">
              ...
              <div className="flex items-center gap-4 mb-6 p-4 rounded-xl bg-muted">
                ...
                <div>
                  ...
                  <button className="mt-2 px-3 py-1 rounded-lg text-xs font-semibold text-primary border border-border hover:bg-muted transition-colors">Change Photo</button>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={field.label}>
                    ...
                    <input type="text" defaultValue={field.value} readOnly={field.readonly} className="w-full px-3 py-2 rounded-lg outline-none bg-muted border border-border text-[13px]" style={{ color: field.readonly ? 'var(--muted-foreground)' : 'var(--foreground)', cursor: field.readonly ? 'not-allowed' : 'text' }} />
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'appearance' && (
            <div className="rounded-2xl p-6 bg-card border border-border">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-3 max-w-xs">
                  {[{ label: 'Light', val: false }, { label: 'Dark', val: true }].map(opt => (
                    <button key={opt.label} onClick={() => { if (darkMode !== opt.val) onToggleDark(); }} className="p-4 rounded-xl flex flex-col items-center gap-2 transition-all"
              ...
              <div>
                ...
                <select className="px-3 py-2 rounded-xl outline-none bg-muted border border-border text-[13px] text-foreground">
              ...
              <div className="rounded-2xl p-6 bg-card border border-border">
                ...
                <div className="space-y-3 max-w-sm">
                  {['Current Password', 'New Password', 'Confirm New Password'].map(field => (
                    <div key={field}>
                      <label className="block text-xs font-semibold text-foreground mb-1.5">{field}</label>
                      <input type="password" placeholder="••••••••" className="w-full px-3 py-2 rounded-xl outline-none bg-muted border border-border text-[13px] text-foreground" />
                    </div>
                  ))}
          ...
          {activeTab === 'system' && role === 'Admin' && (
            <div className="space-y-4">
              ...
              <div className="rounded-2xl p-6" style={{ background: '#FEF2F2', border: '1px solid #FECACA' }}>
                ...
                <button className="btn-destructive">Reset All System Data</button>
              </div>
            </div>
          )}
        </div>

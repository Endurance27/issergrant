...
export function Settings({ role, darkMode, onToggleDark }: SettingsProps) {
  ...
  if (role === "Admin")
    tabs.push({ id: "system", label: "System", icon: <Globe size={15} /> });
          ...
          {activeTab === "profile" && (
            <div className="rounded-2xl p-6 bg-card border border-border">
              ...
              <div className="flex items-center gap-4 mb-6 p-4 rounded-xl bg-muted">
                <div className="flex items-center justify-center w-16 h-16 rounded-full text-white font-extrabold text-[22px] bg-primary">
                  {user.avatar}
                </div>
                <div>
                  ...
                  <input
                    ...
                    accept="image/*"
                    className="hidden"
                    onChange={() => toast("Photo updated", "success")}
                  />
                  <button
                    onClick={() => photoInputRef.current?.click()}
                    className="mt-2 px-3 py-1 rounded-xl text-xs font-semibold text-primary border border-border hover:bg-muted transition-colors"
                  >
                    Change Photo
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={user.role}
                    readOnly
                    className="w-full px-3 py-2 rounded-xl outline-none bg-muted border border-border text-[13px] text-muted-foreground cursor-not-allowed"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1.5">
              ...
              <div className="space-y-1">
                {[
                  ...
                  },
                  {
                    ...
                    sub: "Show notifications in the portal",
                  },
                  {
                    key: "deadlines",
                    label: "Deadline Reminders",
                    sub: "Get reminded about upcoming grant and milestone deadlines",
                  },
                  {
                    key: "approvals",
                    label: "Approval Updates",
                    sub: "Notifications when proposals or reports are approved or rejected",
                  },
                  {
                    key: "payments",
                    label: "Payment Alerts",
                    sub: "Alerts when disbursements are processed or pending",
                  },
                  {
                    key: "system",
                    label: "System Announcements",
                    sub: "Platform maintenance and system-wide updates",
                  },
                ].map((item) => (
                  <div
                    key={item.key}
                  ...
                  >
                    ...
                    >
                      <span
                        className="absolute top-1 size-[16px] rounded-full transition-all duration-200 bg-white"
                        style={{
                          left:
                            notifPrefs[item.key as keyof typeof notifPrefs] ?
                              24
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (darkMode !== opt.val) {
                          onToggleDark();
                          toast(`${opt.label} mode enabled`, "info");
                        }
                      }}
                      className="p-4 rounded-xl flex flex-col items-center gap-2 transition-all"
                      style={{
                        border: `2px solid ${darkMode === opt.val ? "var(--primary)" : "var(--border)"}`,
                        background:
                          darkMode === opt.val ?
                            "var(--secondary)"
                          : "var(--muted)",
                    ...
                    >
                      <div
                        ...
                        style={{ background: opt.val ? "#0F172A" : "#F8FAFC" }}
                      />
                      <span
          ...
          {activeTab === "system" && role === "Admin" && (
            <div className="space-y-4">
              <div className="rounded-2xl p-6 bg-card border border-border">
                  ...
                  {[
                    {
                      ...
                      value:
                        "Institute for Science and Engineering Research (ISER)",
                    },
                    {
                      label: "Portal URL",
                      value: "https://portal.iser.edu.my",
                    },
                ...
                <div className="mt-6">
                  <button
                    onClick={() => save("System settings saved")}
                    className="btn-primary flex items-center gap-2"
                  >
              ...
              <div className="rounded-2xl p-6 bg-red-50 border border-red-200">
                <h3 className="font-bold text-sm mb-2 text-red-900">
                  Danger Zone
                </h3>
                <p className="text-[13px] mb-3 text-red-700">

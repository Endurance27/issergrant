...
import { currentUsers, proposals } from "../../data/mockData";
...
export function Awards({ role, onNavigate }: AwardsProps) {
  ...
  const requestDisbursement = () => {
    if (!showDisburse || !disburseAmount || !disburseDesc) {
      toast("Fill in all fields", "error");
      return;
    }
    const amt = Number(disburseAmount);
    if (amt <= 0 || amt > showDisburse.remaining) {
      toast(
        `Amount must be between 1 and ${fmtCurrency(showDisburse.remaining)}`,
        "error",
      );
      return;
    }
    const tx: Transaction = {
      ...
      requestedBy: currentUsers["Finance Officer"].name,
    };
    addTransaction(tx);
    supabase
      .from("transactions")
    ...
    addNotification({
      ...
      message: `${fmtCurrency(amt)} disbursement request for "${showDisburse.title}" is pending approval.`,
      time: "Just now",
      type: "payment",
    });
    addAuditLog({
      action: "Disbursement Requested",
      user: currentUsers["Finance Officer"].name,
      role,
      module: "Financial",
      timestamp: new Date().toISOString().slice(0, 16).replace("T", " "),
      ip: "192.168.1.1",
      details: `${fmtCurrency(amt)} — ${disburseDesc}`,
    });
  ...
  const createAward = () => {
    if (!newTitle || !newResearcher || !newAmount || !newStart || !newEnd) {
      toast("Fill in all fields", "error");
      return;
    }
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          ...
          value={awards.filter((a) => a.status === "Active").length}
      ...
      {(role === "director" || role === "admin") && (
        <div
          ...
          data-testid="department-allocations"
        >
          ...
          <div className="space-y-4">
            {(() => {
              ...
              awards.forEach((a) => {
                const dept =
                  proposals.find((p) => p.id === a.proposalId)?.department ??
                  "Unassigned";
                byDept.set(dept, (byDept.get(dept) ?? 0) + a.awardedAmount);
              });
        ...
        {awards.map((award) => {
          ...
          const myAward =
            role === "researcher_pi" || role === "researcher_co_pi" ?
              award.researcher === currentUsers[role]?.name
            : false;
          return (
            <div
              key={award.id}
              className="rounded-2xl bg-card border border-border overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200"
              style={{
                borderLeftWidth: 4,
                borderLeftColor: isActive ? "#22C55E" : "#94A3B8",
              }}
            >
              <div className="p-5">
                ...
                <div className="mt-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      className={`font-mono font-bold text-xs ${pct >= 80 ? "text-amber-500" : "text-emerald-500"}`}
                    >
                      {pct}%
                    </span>
                  </div>
          ...
          <div className="space-y-4">
            <div className="p-3 rounded-xl bg-secondary border-l-4 border-primary">
              ...
              <div className="text-xs text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">
                  {fmtCurrency(showDisburse.remaining)}
                </span>
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 rounded-xl outline-none resize-none bg-muted border border-border text-[13px] text-foreground"
              />
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => setShowDisburse(null)}

...
export function AuditLogs() {
  ...
  const filtered = useMemo(() => {
    ...
    const base = auditLogs.filter(l => {
      const matchSearch = !q || l.action.toLowerCase().includes(q) || l.user.toLowerCase().includes(q) || l.details.toLowerCase().includes(q) || l.module.toLowerCase().includes(q);
  ...
  const handleExport = () => {
    const rows = [
      ...
      ...filtered.map(l => [l.timestamp, l.action, l.user, l.role, l.module, l.ip, l.details]),
    ];
        ...
        {modules.filter(m => m !== 'All').map(m => {
          ...
          return (
            <button
              ...
              style={{ background: isActive ? color + '15' : 'var(--card)', borderColor: isActive ? color : 'var(--border)', color: isActive ? color : 'var(--muted-foreground)' }}
            >
              <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: color }} />
              {m}
              <span className="font-mono font-bold ml-0.5" style={{ color }}>{count}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="hidden md:block rounded-2xl overflow-hidden border border-border">
                ...
                return (
                  <tr key={log.id} className="bg-card hover:bg-muted transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded-lg text-[11px] font-semibold whitespace-nowrap" style={{ background: color + '15', color }}>{log.module}</span>
                    </td>
                    <td className="px-4 py-3">
        ...
        {paginated.length === 0 && (
          <div className="flex flex-col items-center justify-center py-14">
            ...
            <div className="font-bold text-sm text-foreground">No matching log entries</div>
            <div className="text-xs text-muted-foreground mt-1">Try adjusting your search or filter</div>
          </div>
        )}
        <Pagination page={page} totalPages={totalPages} total={filtered.length} pageSize={12} onPage={setPage} />
      </div>
      ...
      <div className="md:hidden space-y-2">
          ...
          return (
            <div key={log.id} className="rounded-2xl bg-card border border-border p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <span className="px-2 py-0.5 rounded-lg text-[10px] font-semibold flex-shrink-0" style={{ background: color + '15', color }}>{log.module}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div>
                  <div className="text-[10px] text-muted-foreground">User</div>
                  <div className="text-xs font-semibold text-foreground">{log.user}</div>
                </div>
                <div>
        ...
        {paginated.length === 0 && (
          <div className="flex flex-col items-center justify-center py-14 rounded-2xl border border-dashed border-border">
            <Shield size={28} className="text-muted-foreground opacity-30 mb-3" />
            <div className="font-bold text-sm text-foreground">No log entries</div>
          </div>
        )}
        <Pagination page={page} totalPages={totalPages} total={filtered.length} pageSize={12} onPage={setPage} />
      </div>
